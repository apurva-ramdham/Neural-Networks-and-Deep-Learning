...
def conv2d_forward(x, w, b, pad, stride):
    ...
    for n in range(N):
        x_pad = np.pad(x[n,:,:,:], ((pad,pad),(pad,pad), (0,0)), 'constant')
...
def conv2d_backward(d_top, x, w, b, pad, stride):
    ...
    N, H, W, C = x.shape
    HH, WW, _, F = w.shape
    _, H_prime, W_prime, _ = d_top.shape
    ...
    for n in range(N):
        dx_pad = np.pad(dx[n,:,:,:], ((pad,pad),(pad,pad), (0,0)), 'constant')
        x_pad = np.pad(x[n,:,:,:], ((pad,pad),(pad,pad), (0,0)), 'constant')
        for f in range(F):
            for h_prime in range(H_prime):
                for w_prime in range(W_prime):
                    ...
                    w1 = w_prime * stride
                    w2 = w_prime * stride + WW
                    dx_pad[h1:h2, w1:w2, :] += w[:,:,:,f] * d_top[n,h_prime,w_prime,f]
                    d_w[:,:,:,f] += x_pad[h1:h2, w1:w2, :] * d_top[n,h_prime,w_prime,f]
                    d_b[f] += d_top[n,h_prime,w_prime,f]
    ...
    for n in range(N):
        for h_prime in range(H_prime):
            for w_prime in range(W_prime):
                for c in range(C):
                    h1 = h_prime * stride
                    h2 = h_prime * stride + pool_size
                    w1 = w_prime * stride
                    w2 = w_prime * stride + pool_size

                    window = x[n, h1:h2, w1:w2, c]
                    out[n, h_prime, w_prime, c] = np.max(window)
        ...
        for c in range(C):
            for h in range(H_prime):
                for w in range(W_prime):
                    h1 = h * stride
                    h2 = h * stride + pool_size
                    w1 = w * stride
                    w2 = w * stride + pool_size

                    window = x[n, h1:h2, w1:w2, c]
                    window2 = np.zeros_like(window)
                    window2[np.argmax(window2)] = 1

                    dx[n,h1:h2,w1:w2,c] = window2 * dout[n,h,w,c]
